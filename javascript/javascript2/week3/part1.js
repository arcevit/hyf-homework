...
doubleOddArray=numbers
.filter(value=>value%2!==0)
.map(value=>value*2)


console.log("doubled odd numbers are  ,",doubleOddArray)
...
const aboveRating6=moview
.filter(value=>value.rating>6)
.map(value=>value.rating)
console.log(aboveRating6)


//************************************************** */
let keywords=["The", "dog", "who", "is", "not", "a", "man"];
const title=moview.map(value=>value.title.toLowerCase())
console.log(title)

let n=0;

keywords.forEach((z)=>title.forEach(value=>value.indexOf(z.toLowerCase())>=0 ? n++ : null ))

console.log(n)


//********************************************* */
let totalRates=moview.reduce((passIn,item)=>{ 
        console.log(passIn,item.rating)
      return  passIn+(item.rating)},0)

let totalRate=moview.reduce((total,items)=>total+(items.rating),0)
console.log("total rating of the movies are  ",totalRate)

//****************** */

let total=newmoview.reduce((passIn,item)=>{ 
        //  console.log(passIn,item.tag)
         return passIn+item.tag+",";
